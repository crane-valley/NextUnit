...
- When generating year/date strings:
  ...
  - If not available → assume **January 2026**.
...
- **When adding a new .csproj file**, always update the `NextUnit.slnx`
  file to include the new project.
  - Add the project path in the appropriate folder
    (e.g., `/sample/`, `/test/`, `/benchmark/`).
...
- If these instructions conflict with explicit commands from the user
  or with clear project conventions, **follow the explicit instruction first**.
