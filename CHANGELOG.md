...
- Name the row type in the emitted asynchronous data source call. A `[TestData]` member returning a
  type that implements `IAsyncEnumerable<T>` more than once produced
  ...
  validates against. A source implementing the interface once is unaffected apart from the type
  argument now appearing in the generated file. The task-wrapped shapes are unchanged: their type
  argument is the awaited collection rather than the row, and it was never ambiguous.
