...
## 2026-07-23 candidate audit

| Previous candidate | Decision | Reason |
| ------------------ | -------- | ------ |
| Conditional retry and retry statistics | Re-scoped in `PLANS.md` | Selective transient-failure retry and the current attempt are useful; a mandatory `ShouldRetry()` virtual-method shape and a separate statistics subsystem are not. |
| `MatrixSourceMethod` and `MatrixSourceRange` | Retired | `ValuesFromMember` already accepts a static field, property, or parameterless method; numeric ranges can be returned by that member without another attribute family. |
| Aggregate repeat results | Retired | Each repeat is intentionally a separately addressable test case. Aggregation would hide the failing iteration and duplicate runner reporting. |
| Framework-owned watch mode | Retired | File watching belongs to `dotnet watch`, IDEs, and Microsoft.Testing.Platform tooling rather than the test framework core. |
| Playwright and Aspire packages | Deferred until demand | Generic Playwright/Aspire libraries can use NextUnit lifecycle hooks, artifacts, and `NextUnit.AspNetCore`; first-party wrappers would add version-coupling and feature-parity maintenance without repository demand. |
| Project templates | Narrowed | Keep one C# `dotnet new nextunit` template that proves the one-package path; specialized ASP.NET Core and Playwright templates are unnecessary before demand. |
| NUnit and MSTest migration guides | Retained | They reduce adoption friction at documentation cost only. The automated migration tool is deferred until migration demand is demonstrated. |
| Blazor, Minimal API, and gRPC samples | Retired as standing tasks | The ASP.NET Core sample already proves the integration surface. Add scenario samples in response to concrete usage or bugs. |
| Documentation website | Deferred until scale demands it | The checked-in docs are searchable, versioned with the code, and adequate at the current project scale. |
| Daily benchmarks, AOT benchmarks, generated results, and PR comments | Completed or retired | Daily CI, AOT execution/measurement, generated Markdown/JSON, weekly comparison, and pull-request comments already exist. A second daily comparison schedule would add noise and runner cost. |
| Historical performance tracking | Re-scoped in `PLANS.md` | Durable, noise-aware regression detection is still needed to protect NextUnit's performance claim. |
| Dependency vulnerability scanning | Partially completed | Dependabot and a CI scan exist, but the scan is non-blocking. The remaining task is to make newly introduced known vulnerabilities actionable. |
| CodeQL and SBOM generation | Deferred until risk or distribution requirements change | No open issue, customer requirement, or privileged application surface currently justifies permanent roadmap priority. |
| Culture and locale support | Retained and re-scoped in `PLANS.md` | TUnit, xUnit v3, and NUnit expose culture control; deterministic locale-sensitive tests are a framework-level capability rather than an ecosystem integration. |
