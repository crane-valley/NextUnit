...
public sealed class NextUnitTestDiscoverer : ITestDiscoverer
{
        ...
        ITestCaseDiscoverySink discoverySink)
    {
        ...
        foreach (var vsTestCase in testCases.Select(tc => VSTestCaseFactory.CreateForDiscovery(tc, source)))
        {
        ...
        if (testDataDescriptors is not null && testDataDescriptors.Count > 0)
        {
            ...
            foreach (var vsTestCase in expandedTests.Select(tc => VSTestCaseFactory.CreateForDiscovery(tc, source)))
            {
                discoverySink.SendTestCase(vsTestCase);
            }
        }
